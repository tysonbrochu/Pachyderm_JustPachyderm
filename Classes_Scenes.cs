...
namespace Pachyderm_Acoustic
{
    namespace Environment
    {
        ...
        public abstract class Scene
        {
            ...
            public abstract bool shoot(Point Start, Vector Dir, int Random, out double u, out double v, out int Srf_ID, out List<Point> X_PT, out List<double> t, out List<int> code);
            ...
            ///// <param name="i">surface index</param>
            ///// <returns></returns>
            //public abstract Vector Normal(double u, double v, int i);
            /// <summary>
            ...
            /// <param name="SP_PARAM">a parameter describing some aspect of the partition</param>
            public abstract void partition(List<Point> P, int SP_PARAM);
            /// <summary>
            ...
            /// <param name="P">points to add to the spatial partition system.</param>
            /// <param name="SP_PARAM">a parameter describing some aspect of the partition</param>
            public abstract void partition(Point[] P, int SP_PARAM);
            /// <summary>
            ///a parameter describing some aspect of the partition
            /// </summary>
            /// <param name="P">points to add to the spatial partition system.</param>
            ...
            //public abstract Point ClosestPt(Point P, ref double D);
            /// <summary>
            ...
            //public void Register_Edges(IEnumerable<Point> S, IEnumerable<Point> R)
            //{
            //    List<Hare.Geometry.Point> HS = new List<Hare.Geometry.Point>();
            //    List<Hare.Geometry.Point> HR = new List<Hare.Geometry.Point>();

            //    foreach (Point SPT in S) HS.Add(Utilities.PachTools.RPttoHPt(SPT));
            //    foreach (Point RPT in R) HR.Add(Utilities.PachTools.RPttoHPt(RPT));
            ...
            //    foreach (Point RPT in R.Origins()) HR.Add(Utilities.PachTools.RPttoHPt(RPT));
        ...
        public class Polygon_Scene : Scene
        {
            ...
            //        {
            //            B.Add(((Brep)Breps[q]).DuplicateBrep());
            ...
            //        {
            //            Brep BObj = ((Extrusion)Breps[q]).ToBrep();
            ...
            //        meshes = Mesh.CreateFromBrep(BList[q], MeshingParameters.Smooth);
            ...
            //                    Point FP = meshes[t].Vertices[meshes[t].Faces[u][0]];
            ...
            //        {
            //            Point pt = Utilities.PachTools.HPttoRPt(Topo[0].Polygon_Centroid(Topo[0].Plane_Members[i][j]));
            ...
            //        {
            //            Brep BObj = ((Rhino.DocObjects.ExtrusionObject)ObjectList[q]).ExtrusionGeometry.ToBrep();
            //            for (int i = 0; i < BObj.Faces.Count; i++)
            //            {
            //                if (ObjectList[q].Geometry.GetUserString("Acoustics_User") == "yes")
            //                {
            ...
            //            meshes = Mesh.CreateFromBrep((Brep)BrepList[BrepList.Count - 1], mp);
            //            if (meshes == null) throw new Exception("Problem with meshes");
            ...
            //        mp.MinimumEdgeLength = 0.1;
            //        mp.SimplePlanes = true;
            //        meshes = Mesh.CreateFromBrep((Brep)BList[q],mp);
            //        if (meshes == null) throw new Exception("Problem with meshes");
            ...
            //private bool coplanarmesh(Mesh M)
            //{
            ...
            //                Point pt = Naked_Edges[shortid].PointAt(tsS);
            ...
            //                if (Math.Abs(Vector.Multiply(Naked_Edges[shortid].TangentAt(tsS), Naked_Edges[0].TangentAt(tlS))) < 0.98)
            //                {
            ...
            //                Curve[] crv;
            //                Point[] pts;
            //                Intersect.Intersection.CurveBrep(Naked_Edges[0], BrepList[i], 0.01, out crv, out pts);
            //            }
            ...
            public Hare.Geometry.Point Image(Hare.Geometry.Point Point, int Top_Id, int q)
            {
                ...
                //Rhino.RhinoDoc.ActiveDoc.Objects.Add(new LineCurve(Utilities.PachTools.HPttoRPt(PX), Utilities.PachTools.HPttoRPt(Topo[0].Polygon_Centroid(q))));
            ...
            //public void partition(List<Point> P)
            //{
            //    Partitioned = true;
            //    List<Hare.Geometry.Point> PTS = new List<Hare.Geometry.Point>();
            //    foreach (Point PT in P)
            ...
            public override bool shoot(Point Start, Vector Dir, int Random, out double u, out double v, out int Poly_ID, out List<Point> X_PT, out List<double> t, out List<int> code)
            {
                ...
                {
                    X_PT = new List<Point>();
                    foreach (Hare.Geometry.Point P in X) X_PT.Add(new Point(P.x, P.y, P.z));
                    return true;
                }
                X_PT = new List<Point> { default(Point) };
                return false;
            }
            ...
            public override bool shoot(Hare.Geometry.Ray R, out double u, out double v, out int Poly_ID, out Hare.Geometry.Point X_PT, out double t)
            {
                ...
                //Rhino.RhinoDoc.ActiveDoc.Objects.Add(new LineCurve(Utilities.PachTools.HPttoRPt(R.origin), Utilities.PachTools.HPttoRPt(R.origin + R.direction)));
                t = 0;
                u = 0;
                v = 0;
                return false;
            ...
            //public override Point ClosestPt(Point P, ref double Dist)
            //{
            //    Hare.Geometry.Point PH = new Hare.Geometry.Point(P.x, P.y, P.z);
            //    PH = ClosestPt(PH, ref Dist);
            //    return new Point(PH.x, PH.y, PH.z);
            //}
